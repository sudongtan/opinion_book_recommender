...
class chainRec(object):
    ...
    def model_constructor(self, n_user, n_item, n_stage, HIDDEN_DIM, LAMBDA, LEARNING_RATE):
        ...
        embs = np.load(f'./data/books_embs_{self.HIDDEN_DIM}.npy')
    
        user_emb = tf.get_variable("user_emb", [n_user, HIDDEN_DIM], 
                                   initializer=tf.random_uniform_initializer(-0.01, 0.01))
        item_emb = tf.get_variable(name="item_emb", shape=embs.shape, initializer=tf.constant_initializer(embs))
        ...
        for _k0 in range(0, data_test_slice.shape[0], BATCH_SIZE):
            ...
            for _ki in range(len(user_list)):
                ...
                if data_test_slice[_k,2] >= stage_id:
                    ...
                    _hr = int(wrong < topK)
                    metric_vali_slice.append([stage_id, _auc, _ndcg, _hr])
                # test
                if data_test_slice[_k,4] >= stage_id:
                    ...
                    _ndcg = 1.0/np.log2(2 + wrong)
                    _hr = int(wrong < topK)
                    metric_test_slice.append([int(stage_id), _auc, _ndcg, _hr])
        print("complete!")
    ...
    def evaluation(self, data_test, user_item_map, topK):
        ...
        with tf.Graph().as_default(), tf.Session() as session:
            ...
            index_list = (data_test['max_stage_vali']>=target_stage_id-1) + (data_test['max_stage_test']>=target_stage_id-1)
            data_test_slice = data_test[index_list].values
            metric_vali_1, metric_test_1 = self.evaluate_model_slice(u, s, target_stage_id-1, data_test_slice, user_item_map, topK=topK)

        res = np.array([metric_vali.mean(axis=0), metric_test.mean(axis=0), metric_vali_1.mean(axis=0), metric_test_1.mean(axis=0)])
        print('Stage 2')
        print("AUC={:.3f}, NDCG={:.3f} (validation)".format(res[0,1], res[0,2]))
        print("AUC={:.3f}, NDCG={:.3f} (test)".format(res[1,1], res[1,2]))
        print('Stage 1')
        print("AUC={:.3f}, NDCG={:.3f} (validation)".format(res[2,1], res[2,2]))
        print("AUC={:.3f}, NDCG={:.3f} (test)".format(res[3,1], res[3,2]))
